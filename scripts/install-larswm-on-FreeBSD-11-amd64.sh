...
echo "exec /usr/local/bin/larswm" >> ~/.xsession
chmod +x ~/.xsession

cp /usr/local/share/doc/larswm/sample.larswm /root/.larswm
find /usr/home -type d -maxdepth 1 | grep "/usr/home/.*" |grep -v -e '^$' | xargs cp /root/.xsession
find /usr/home -type d -maxdepth 1 | grep "/usr/home/.*" |grep -v -e '^$' | xargs cp /root/.larswm
find /usr/home -maxdepth 1 -type d | cut -d '/' -f 4 | grep -v -e '^$' | grep -v 'lost+found' | xargs -I {} chown {}:{} /usr/home/{}/.xsession
find /usr/home -maxdepth 1 -type d | cut -d '/' -f 4 | grep -v -e '^$' | grep -v 'lost+found' | xargs -I {} chown {}:{} /usr/home/{}/.larswm
